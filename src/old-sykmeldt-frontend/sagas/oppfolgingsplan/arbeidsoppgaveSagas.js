...
export function* lagreArbeidsoppgave(action) {
  ...
  try {
    yield put(
      actions.lagrerArbeidsoppgave(
        action.fnr,
        action.arbeidsoppgave.arbeidsoppgaveId
      )
    );
    ...
    yield put(
      actions.arbeidsoppgaveLagret(
        action.id,
        data,
        action.arbeidsoppgave,
        action.fnr
      )
    );
  } catch (e) {
...
export function* slettArbeidsoppgave(action) {
  try {
    ...
    yield put(
      actions.slettArbeidsoppgaveFeilet(action.fnr, action.arbeidsoppgaveId)
    );
  }
}
