...
import {HakeGronnLysImage, HakeOransjeImage, KryssRoedImage, VarseltrekantImage} from '@/common/images/imageComponents';
    ...
    return (
        <div>
            ...
            {arbeidsoppgaver.map((arbeidsoppgave, idx) => {
                return (
                    <div className={`godkjennPlanOversiktInformasjon__panel__arbeidsoppgave ${hentPanelType}`}
                         key={idx}>
            ...
            {tiltakListe.map((tiltak, idx) => {
                return (
                    <div className="godkjennPlanOversiktInfo__panel__tiltak" key={idx}>
                        ...
                        {tiltak.status && (
                            <Tag size={"small"}
                                 variant={tiltak.status === STATUS_TILTAK.AVTALT ? "success" : "warning"}>
